...
public class ShawnWarningAdapter extends BaseAdapter{
	...
	private SimpleDateFormat sdf1 = new SimpleDateFormat("yyyyMMddHHmmss", Locale.CANADA);
	private SimpleDateFormat sdf2 = new SimpleDateFormat("yyyy-MM-dd HH:mm", Locale.CANADA);
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			if (!isMarkerCell) {
				...
				convertView = mInflater.inflate(R.layout.shawn_adapter_warning_map, null);
			}
		...
		if (!TextUtils.isEmpty(dto.name)) {
			if (!isMarkerCell) {
				...
				}else if (dto.name.contains("更新")) {
					mHolder.tvName.setText(dto.name.replace("更新", "更新"+"\n"));
				}
			}
		}
